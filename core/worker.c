...
static void work_timer_func(struct timer *t __unused, void *data, uint64_t __unused now)
{
...
struct worker *new_worker(worker_entry_t entry, void *arg)
{
	...
	w->frame = w->stack + STACK_SIZE - sizeof(struct stack_frame) - 0x100;
	w->frame->pc = *((uint64_t *) entry); // dereference the OPD
	w->frame->gpr[2] = (uint64_t) __toc;
